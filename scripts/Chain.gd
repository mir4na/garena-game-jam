...
func reset_rope() -> void:
	rope_points.clear()
	queue_redraw()
	# Forces are reset every frame in physics_process, so no need to reset them here explicitly
	# unless we want to clear any accumulated impulse, but Player handles that.
